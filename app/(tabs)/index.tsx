...
const TaskSection = ({ title, tasks, checkedTasks, onToggleTask }: { 
  title: string, 
  tasks: string[], 
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, gestureState) => {
        if (gestureState.dy > 0) { // Only react to downward swipes
          // Follow the finger
          animatedHeight.setValue(modalHeight - gestureState.dy);
        }
      },
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dy > 50) { // If pulled down more than 50px
          // Close the modal
          ...
          <View style={styles.optionSection}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScrollView}>
              {CATEGORIES.map(category => (
                ...
                >
                  <Text 
                    style={[styles.categoryChipText, selectedCategory === category && styles.selectedChipText]}
                  >
                    #{category}
                  </Text>
                </TouchableOpacity>
          ...
          <View style={styles.optionSection}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.optionsScrollView}>
              {PROJECTS.map(project => (
                <TouchableOpacity 
                  key={project} 
                  style={[styles.projectChip, selectedProject === project && styles.selectedChip]}
                  onPress={() => setSelectedProject(project)}
                >
                  <FontAwesome name="folder" size={12} color={selectedProject === project ? '#000' : '#CBFF00'} style={styles.chipIcon} />
                  <Text 
                    style={[styles.projectChipText, selectedProject === project && styles.selectedChipText]}
                  >
                    {project}
                  </Text>
                </TouchableOpacity>
          ...
          <View style={styles.optionSection}>
            ...
            <View style={styles.priorityContainer}>
              {PRIORITIES.map(priority => (
                <TouchableOpacity 
                  key={priority.id} 
                  style={[styles.priorityChip, selectedPriority === priority.id && { backgroundColor: priority.color }]}
                  onPress={() => setSelectedPriority(priority.id)}
                >
                  <Text 
                    style={[styles.priorityChipText, selectedPriority === priority.id && styles.selectedPriorityText]}
                  >
